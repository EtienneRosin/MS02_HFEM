...
from hfem.problems import PeriodicPoissonProblem
...
if __name__ == '__main__':
    ...
    periodic_pb = PeriodicPoissonProblem(mesh, pb_config)

    # Solve the problem
    periodic_pb.solve()
    
    # Display solution (and error if exact solution is provided)
    periodic_pb.display_solution(
        solution_config(
            kind='trisurface',
            save_name = f'study_cases/III_Periodic/periodic_solution.pdf'
        )
    )
    plt.show()
    ...
    periodic_pb.display_error(
        error_config(
            ...
            cbar = True,
            save_name = f'study_cases/III_Periodic/periodic_absolute_error.pdf'
        )
    )
    plt.show()
